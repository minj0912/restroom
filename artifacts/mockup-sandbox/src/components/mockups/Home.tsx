...
import { Sparkles, Construction, Box, ShieldCheck, Bug, Car, Bell } from 'lucide-react';
...
const CATEGORIES = [
  ...
  { id: "supplies", title: "비품 부족", subtitle: "소모품 요청", icon: Box, color: "#10B981", bg: "#ECFDF5" },
  { id: "safety", title: "안전 신고", subtitle: "위험 요소 신고", icon: ShieldCheck, color: "#EF4444", bg: "#FEF2F2" },
...
export default function Home() {
  return (
    <div className="min-h-screen bg-slate-50 font-sans p-5 pb-24">
      {/* Header */}
      ...
      <div className="bg-blue-600 rounded-3xl p-6 mb-8 text-white flex justify-between items-center shadow-lg shadow-blue-200">
        ...
        <div className="opacity-40"><Construction size={56} /></div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {CATEGORIES.map((item) => (
          <div key={item.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-50 hover:border-blue-200 transition-all active:scale-95 cursor-pointer">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4" style={{ backgroundColor: item.bg }}>
              <item.icon size={24} style={{ color: item.color }} />
            </div>
            <p className="font-bold text-slate-900 mb-1" style={{ color: item.color }}>{item.title}</p>
